...
- Rails에서는 CRUD를 구현할 때 MVC를 각각 구현하는 것이 아니라 명령어로 쉽게 구현하는 방식을 제공하고 있다. 그 방식이 바로 `Scaffolding`(스캐폴딩) 이다.
 
## Scaffolding을 사용하는 주 상황
### 1. Rails 코드의 정석을 학습하기 위해
### 2. 빠르게 동작하는 프로젝트를 만들고 싶을 때
### 3. 로직이 일반적인 CRUD를 크게 벗어나지 않을 때

실제 `scaffold`를 만들어보고, RESTful이라는 개념에 대해서 이야기해보자.
...
- scaffold를 생성하는 명령어가 마치 모델을 생성하는 것과 비슷하다고 느껴질 수 있다.
- 단, scaffold는 모델뿐만 아니라 CRUD 기능을 하는 view,controller,routes도 자동 생성한다는 점에서 차이가 있다.
  
controller, view파일등이 자동으로 설정되어지고, routes.rb에는 root 경로만 입력해준다.
```Ruby
# config/routes.rb
Rails.application.routes.draw do
    # resources :posts 코드위에서 root 주소를 설정.
    root "posts#index"
